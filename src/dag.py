...
def generate_dag():
    ...
    for task_name in edge_task_names:
        edge_provider = random.choice(['Cluj-MdC-01', 'Sibiu-MdC-02', 'Timisoara-MdC-03'])
        dag.add_node(task_name, type='EdgeTask', region='eu-central-1',
                     ...
                     energy_consumption=random.uniform(20, 40))  # in Watts
    ...
    for task_name in fog_task_names:
        ...
        dag.add_node(task_name, type='FogTask', region='eu-central-1',
                     provider=fog_provider,
                     data_volume=random.uniform(10, 50),  # in GB
                     computational_requirements={'CPU': random.uniform(10, 30), 'Memory': random.uniform(30, 50)},  # in GHz, GB
                     latency=random.uniform(5, 10),  # in ms
                     bandwidth=random.uniform(10, 20),  # in Mbps
                     energy_consumption=random.uniform(30, 60))  # in Watts
    ...
    for task_name in cloud_task_names:
        ...
        dag.add_node(task_name, type='CloudTask', region=region,
                     provider=provider,
                     data_volume=random.uniform(50, 100),  # in GB
                     computational_requirements={'CPU': random.uniform(30, 50), 'Memory': random.uniform(50, 100)},  # in GHz, GB
                     latency=random.uniform(10, 20),  # in ms
                     bandwidth=random.uniform(20, 30),  # in Mbps
                     energy_consumption=random.uniform(60, 80))  # in Watts
    ...
    for edge_task in edge_task_names:
        ...
        for fog_task in random.sample(fog_task_names, random.randint(1, len(fog_task_names))):
            dag.add_edge(edge_task, fog_task)

    for fog_task in fog_task_names:
        # Connect fog tasks to a random subset of cloud tasks
        for cloud_task in random.sample(cloud_task_names, random.randint(1, len(cloud_task_names))):
...
node_properties = {node: f"{node}, Type: {dag.nodes[node]['type']}, region: {dag.nodes[node]['region']}, "
...
for node, properties in node_properties.items():
    ...
    color = 'green' if node_type == 'EdgeTask' else 'blue' if node_type == 'FogTask' else 'purple'
